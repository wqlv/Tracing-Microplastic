...
from keras.layers import Input, Conv1D, Concatenate, Flatten, Dense, Dropout
...
def build_model():
    ...
    x1 = MobileNet(weights='imagenet', include_top=False, input_shape=(256, 256, 3))(inputs1)
    x1 = Flatten()(x1)
    x1 = Dense(512, activation='relu')(x1)
    x1 = Dropout(0.5)(x1)  # Add Dropout layer
    print(x1.shape)

    x2 = Conv1D(filters=16, kernel_size=2, activation='relu', padding='same')(inputs2)
    x2 = Conv1D(filters=32, kernel_size=3, activation='relu', padding='same')(x2)
    x2 = Flatten()(x2)
    x2 = Dense(512, activation='relu')(x2)
    x2 = Dropout(0.5)(x2)  # Add Dropout layer
    print(x2.shape)
...
filepath = "modelmobile.h5"
checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max', save_freq='epoch')
history = model.fit([x_train1, x_train2], y_train1, validation_data=([x_test1, x_test2], y_test1), epochs=  , batch_size= , callbacks=[checkpoint])
